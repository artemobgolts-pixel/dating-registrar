...
RATE_RULES = {
    ...
    "review-decline": (10, 30, 600), # отложить отзыв в «Ждут отзыва»
    "name": (10, 30, 600),        # смена имени
    "report": (5, 20, 600),       # 5 жалоб за 10 минут (анти-спам очереди)
    "pin": (5, 20, 600),          # перебор 4-значного PIN подборки
}

# Лимиты на залогиненного создателя (ключ — user_id). Помимо общей квоты
# date_limit (по умолчанию 100 лично созданных активных событий) дополняет
# краткосрочную защиту от всплесков:
# kind: (лимит на пользователя, окно в секундах)
USER_RATE_RULES = {
    "datecreate": (40, 3600),     # 40 событий в час (общую квоту это не отменяет)
    "dateedit": (120, 3600),      # 120 правок событий в час
    "datebulk": (40, 3600),      # 40 массовых действий в час, до 100 событий в каждом
    "community_search": (60, 60), # поиск тяжелей обычной страницы: 60 запросов/мин
}

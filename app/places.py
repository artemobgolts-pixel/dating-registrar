...
def process_place(place: str | None) -> tuple[str | None, str | None]:
    ...
    чтобы не блокировать сохранение события на запросе к картам.
    """
...
def resolve_into_db(date_id: int, url: str) -> None:
    """Фон: тянет название из <title> и дописывает в запись события.
...
def repair_legacy_places() -> int:
    """Чинит старые события, где ссылка осела прямо в поле place.

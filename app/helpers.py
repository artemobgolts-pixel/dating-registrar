...
# Длина токена ссылки в байтах. 8 байт = 64 бита энтропии и примерно 11
# символов base64url. Уже выданные более длинные токены остаются валидными.
LINK_TOKEN_BYTES = 8


def new_link_token() -> str:
    """Короткий непредсказуемый токен для ссылок категорий и шаринга событий.
...
def fmt_ymaps(place: str | None) -> str:
    ...
    Это чинит старые события, у которых ссылка осела в place, а place_url
...
def plural(n: int, one: str, few: str, many: str) -> str:
    """Русские склонения: 1 событие, 2 события, 5 событий."""
    n = abs(int(n))
...
# Варианты оплаты события (колонка pay_split): 0 — не указано, 1 — 50/50,
# 2 — платит автор, 3 — платит гость, 4 — участие бесплатное.
PAY_LABELS = {
    1: "💸 50/50",
    2: "👌 Я плачу",
    3: "🫵 Ты платишь",

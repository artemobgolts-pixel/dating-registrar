...
vec2 inkDens(vec2 p, float sd, float warp, float evo){
  ...
  vec2 wp = p;
  wp += warp * 0.60 * (vec2(fbm(p*1.4 + sd + evo),        fbm(p*1.4 + sd + 7.0 - evo))  - 0.5);
  wp += warp * 0.45 * (vec2(fbm(wp*3.0 - sd*1.2 + evo*1.6), fbm(wp*3.0 + sd + 3.0 - evo*1.6)) - 0.5);
  wp += warp * 0.26 * (vec2(fbm(wp*6.5 + sd*2.0 + evo*2.4), fbm(wp*6.5 - sd + 9.0 + evo*2.4)) - 0.5);
  float rr = length(wp);
...
vec3 inkAt(vec3 col, vec2 uv, vec2 dsp, vec2 cc, float age, float sd){
  ...
  vec2 d = uv - cc;
  // тушь стоит РОВНО на месте: поле смещения воды (dsp) больше не подмешиваем.
  // При движении курсора dsp неравномерно по площади пятна и сдвигало разные
  // его части по-разному — клякса выгибалась в «банан». В статич. эталоне dsp=0,
  // там изгиба нет; обнуляем снос и в живой симуляции, чтобы форма совпала.

...
(function () {
  ...
  var PERSIST    = 0.9988;             // медленное «зарастание» следа (~10с полузатух.) фоном, без натяжения
...
const vec3 EMBER = vec3(0.945, 0.553, 0.286);  // тёплый оранжевый след курсора
...
void main(){
  ...
  col = mix(col, EMBER, clamp(dsp.z, 0.0, 1.0) * 0.20);   // тёплый оранжевый след курсора (чуть-чуть)
  o = vec4(col, 1.0);
}`;

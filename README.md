...
|-----|-----|----------|
| `@` | A   | IP твоего сервера |
| `www` | A | тот же IP сервера |

`www.date4you.online` автоматически перенаправляется на канонический домен без
`www`, поэтому обе записи должны указывать на один сервер.

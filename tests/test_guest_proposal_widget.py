...
class GuestProposalWidgetContractTests(unittest.TestCase):
    def test_guest_copy_and_accessibility_are_consistent(self):
        ...
        self.assertIn('class="btn book vote-ended"', template)
        self.assertIn("vote_state.status != 'unconfigured'", template)
        self.assertIn("voting_accepts_votes", template)
        self.assertNotIn('class="btn book vote-ended"', share_template)
        self.assertNotIn("vote_state", share_template)
        self.assertNotIn("voting_accepts_votes", share_template)
